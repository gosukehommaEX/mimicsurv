...
# First calculate expected events from sample sizes and median survival times
sample_sizes <- c(100, 150)
MST_subgroups <- c(6, 7.5)
follow_up_time <- 24  # months

# Calculate expected events (theoretically correct approach)
expected_events <- calculate_expected_events(sample_sizes, MST_subgroups, follow_up_time)
print(paste("Expected events:", round(expected_events, 1)))
...
where: - $p_j = \frac{E_j}{\sum_{i=1}^{k} E_i}$ is the mixing probability for subgroup $j$ based on expected events - $\lambda_j = \frac{\ln(2)}{\text{MST}_j}$ is the hazard rate for subgroup $j$ - $E_j$ is the expected number of events in subgroup $j$ - $\text{MST}_j$ is the median survival time for subgroup $j$

#### Expected Events Calculation

The expected number of events for each subgroup is calculated as: $$E_j = n_j \times [1 - \exp(-\lambda_j \times t_{\text{follow-up}})]$$
...
This requires numerical methods since the equation: $$\sum_{j=1}^{k} p_j \exp(-\lambda_j t_{median}) = 0.5$$
...
expected_events_3 <- calculate_expected_events(sample_sizes_3, MST_subgroups_3, follow_up_time = 24)
...
### Backward Compatibility: Using Sample Sizes Directly

For convenience, you can also use the `mstfromExpdists_samples()` wrapper function that converts sample sizes to expected events:

```{r mixed-exponential-wrapper}
# Using the wrapper function for backward compatibility
result_wrapper <- mstfromExpdists_samples(
  sample_sizes = c(100, 150),
  MST_subgroups = c(6, 7.5),
  follow_up_time = 24
)

print(result_wrapper$subgroup_summary)
cat("Overall MST (wrapper):", round(result_wrapper$MST_overall, 2), "months\n")
```
...
Combining both contributions: $$\text{Person-time}_i = n_{\text{risk}, i+1} \times \Delta t + (n_{\text{risk}, i} - n_{\text{risk}, i+1}) \times \frac{\Delta t}{2}$$

Simplifying: $$\text{Person-time}_i = \frac{(n_{\text{risk}, i} + n_{\text{risk}, i+1}) \times \Delta t}{2}$$

This is the **trapezoidal rule** for numerical integration.

### Hazard Rate Estimation

Once person-time is calculated, the hazard rate for interval $i$ is estimated as: $$\hat{\lambda}_i = \frac{n_{\text{events}, i}}{\text{Person-time}_i}$$

### Event Calculation

The number of events in interval $i$ is estimated as: $$n_{\text{events}, i} = n_{\text{risk}, i} - n_{\text{risk}, i+1} - n_{\text{censored}, i}$$

where $n_{\text{censored}, i}$ is the number of individuals censored during interval $i$.

### Median Survival Time Calculation

For piecewise exponential survival, the median is found by solving: $$S(t_{\text{median}}) = 0.5$$
...
-   `calculate_expected_events()`: Calculate expected events from sample sizes and median survival times
-   `mstfromExpdists_samples()`: Wrapper function for backward compatibility with sample sizes
